		...
		for (const p of plateaus) {
			for (const child of p.children) {
				...
				for (const ch of child.paramChanges) {
					let cat = 'other';
					if (['depth', 'model_dim', 'n_heads', 'head_dim', 'window_pattern'].includes(ch.key)) cat = 'architecture';
					else if (ch.key.endsWith('_lr')) cat = 'learning rate';
					else if (ch.key.includes('batch')) cat = 'batch size';
					else if (ch.key === 'warmdown_ratio') cat = 'scheduling';
					else if (ch.key === 'weight_decay') cat = 'regularization';
