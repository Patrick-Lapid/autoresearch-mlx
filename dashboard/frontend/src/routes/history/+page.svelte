<script lang="ts">
	...
	import { fetchExperiments, fetchDiff } from '$lib/api';
	...
	async function onSelectExp(exp: Experiment | null) {
		selectedExp = exp;
		selectedDiff = '';
		if (exp) {
			loadingDiff = true;
			selectedDiff = await fetchDiff(exp.id);
			loadingDiff = false;
		}
	}
...
<div class="decisions-view">
	<div class="graph-area">
		<RunGraph {experiments} onSelect={onSelectExp} />
	</div>
	...
	{#if selectedExp}
		<div class="detail-card">
			...
			{#if selectedExp.bpb_at_checkpoints?.length > 0}
				<div class="dc-convergence">
					<span class="dc-conv-label">convergence</span>
					<div class="dc-conv-points">
						{#each selectedExp.bpb_at_checkpoints as bpb, i}
							<span class="dc-conv-point">
								<span class="dc-conv-pct">{(i + 1) * 25}%</span>
								<span class="dc-conv-val">{bpb.toFixed(3)}</span>
							</span>
						{/each}
					</div>
					{#if selectedExp.still_improving}
						<span class="dc-still-improving">still improving at budget end</span>
					{/if}
				</div>
			{/if}
			...
			<div class="dc-metrics">
				<span>{selectedExp.num_params_M.toFixed(1)}M params</span>
				<span>{selectedExp.num_steps} steps</span>
				<span>{selectedExp.training_seconds.toFixed(0)}s train</span>
				<span>{selectedExp.tokens_per_second.toLocaleString()} tok/s</span>
				...
				<span>{selectedExp.gpu_name}</span>
			</div>
			...
			{#if selectedDiff}
				<details class="dc-diff-details">
					...
					<div class="dc-diff"><DiffViewer diff={selectedDiff} /></div>
				</details>
			{:else if loadingDiff}
...
<style>
	...
	.detail-card {
		...
		max-height: calc(100vh - 56px - 6rem); overflow-y: auto;
	}
	...
	.dc-diff-details { border-top: 1px solid var(--border); padding-top: 0.4rem; }
	...
	.dc-loading { font-size: 0.72rem; color: var(--text-dim); padding: 0.25rem 0; }
</style>
