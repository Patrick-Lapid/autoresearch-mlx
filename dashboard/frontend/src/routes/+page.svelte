	...
	{#if latest}
		...
		<section class="stat-tiles">
			...
			<div class="tile">
				...
				<span class="tile-value">{latest.gpu_name}</span>
			</div>
		</section>
		...
		<section class="card">
			...
			<div class="config-grid">
				<div class="config-item"><span class="config-key">depth</span><span class="config-val">{latest.depth}</span></div>
				<div class="config-item"><span class="config-key">model_dim</span><span class="config-val">{latest.model_dim}</span></div>
				<div class="config-item"><span class="config-key">n_heads</span><span class="config-val">{latest.n_heads}</span></div>
				<div class="config-item"><span class="config-key">head_dim</span><span class="config-val">{latest.head_dim}</span></div>
				<div class="config-item"><span class="config-key">window</span><span class="config-val">{latest.window_pattern}</span></div>
				<div class="config-item"><span class="config-key">batch</span><span class="config-val">{latest.total_batch_size}</span></div>
				<div class="config-item"><span class="config-key">matrix_lr</span><span class="config-val">{latest.matrix_lr}</span></div>
				<div class="config-item"><span class="config-key">embed_lr</span><span class="config-val">{latest.embedding_lr}</span></div>
				<div class="config-item"><span class="config-key">wd</span><span class="config-val">{latest.weight_decay}</span></div>
				<div class="config-item"><span class="config-key">warmdown</span><span class="config-val">{latest.warmdown_ratio}</span></div>
			</div>
		</section>
	{/if}
</div>
...
<style>
	...
	.config-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(150px, 1fr)); gap: 0.5rem; }
	.config-item { display: flex; flex-direction: column; gap: 0.15rem; padding: 0.5rem 0.6rem; background: var(--bg-subtle); border-radius: var(--radius-sm); }
	.config-key { font-size: 0.7rem; color: var(--text-dim); font-weight: 500; text-transform: uppercase; letter-spacing: 0.03em; }
	.config-val { font-family: var(--font-mono); font-size: 0.82rem; font-weight: 500; color: var(--text); }
</style>
